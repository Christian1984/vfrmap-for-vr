...
var proc_SimConnect_RequestSystemState *syscall.LazyProc
...
func (s *SimConnect) RequestSystemState(requestID DWORD, szState string) error {
	...
	_szState := []byte(szState + "\x00")

	args := []uintptr{
		uintptr(s.handle),
		uintptr(requestID),
		uintptr(unsafe.Pointer(&_szState[0])),
	}

	r1, _, err := proc_SimConnect_RequestSystemState.Call(args...)
	if int32(r1) < 0 {
		return fmt.Errorf(
			"proc_SimConnect_RequestSystemState for requestID %d szState %s error: %d %s",
			requestID, szState, r1, err,
		)
	}
